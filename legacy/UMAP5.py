...
def load_and_enrich_data_with_grandparent(gp_path, p_path, c_path):
    ...
    try:
        ...
        for p_id, p_row in df_p.iterrows():
            ...
            df_c.loc[mask, 'grandparent_id'] = p_row['grandparent_id'] # 부모의 조부모 ID를 자식에게 연결
...
def universal_callback(run_clicks, clickData, analysis_data, rs_min, rs_max, p_min, p_max, s_min, s_max, direction,
    ...
    if triggered_id == 'main-chart' and clickData:
        try:
            ...
            return dash.no_update, fig_p, fig_gp, f"Child #{clicked_child_id}"

        except (KeyError, ValueError) as e:
            return dash.no_update, placeholder.update_layout(title_text=f"스냅샷 오류: {e}"), placeholder, "오류"
            
    elif triggered_id == 'run-button':
        # (UMAP 생성 로직은 이전과 거의 동일, hover_texts만 수정)
        ...
        if len(df_filtered) < 2:
            return placeholder.update_layout(title_text='데이터 부족'), placeholder, placeholder, "데이터 부족"
        ...
        hover_texts = [
            f"ID:{idx}<br>Force:{_f(r.force_score)}<br>Parent:{int(r.parent_id)} (F:{_f(r.parent_force_score)})<br>G-Parent:{int(r.grandparent_id)} (F:{_f(r.grandparent_force_score)})"
...
if __name__ == '__main__':
    print("UMAP 분석기 (조부모 계층 추가 버전) 실행")
